...
# Feature Extraction
features = df[['product','Product_name','Color']].astype(str).apply(lambda x: ' '.join(x), axis=1)
...
product_index = df[df['product'] == product_of_interest].index[0]
...
similar_products_indices = cosine_similarities[product_index].argsort()[:-2:-1]
similar_products = df.iloc[similar_products_indices]['Product_name'].values
