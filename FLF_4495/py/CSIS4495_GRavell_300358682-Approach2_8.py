...
cosine_similarities = cosine_similarity(specific_product_vector, feature_vectors)

# Create a DataFrame to store similarity results
similarities_df = pd.DataFrame({
    "Product_index": data_encoded.index,  # Use DataFrame index as product index
    "Cosine_similarity": cosine_similarities[0]  # Cosine similarities for all products
})
...
# Filter out the specified product from the results
similarities_df = similarities_df[similarities_df["Product_index"] != specific_product_index]
